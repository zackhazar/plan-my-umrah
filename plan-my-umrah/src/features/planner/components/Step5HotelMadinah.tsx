...
export function Step5HotelMadinah() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h2 className="text-3xl font-heading font-bold text-white mb-2 tracking-wide">Akomodasi Madinah</h2>
        <p className="text-white/50 text-sm">Pilih hotel Anda di Madinah. Kedekatan dengan Masjid Nabawi sangat menentukan kenyamanan ibadah Anda.</p>
      </div>
        ...
        <div className="bg-white/[0.035] p-5 rounded-2xl border border-white/5 flex flex-col justify-center">
          ...
          <div className="flex items-center gap-4">
            <Button 
              ...
              onClick={() => updateHotelMadinah({ nights: Math.max(1, currentNights - 1) })}
            >
              -
            </Button>
            ...
            <Button 
              variant="outline" 
              className="border-white/10 hover:bg-white/5 text-white h-12 w-12 rounded-xl"
              onClick={() => updateHotelMadinah({ nights: currentNights + 1 })}
            >
              +
            </Button>
          </div>
        ...
        <div className="bg-white/[0.035] p-5 rounded-2xl border border-white/5">
          ...
          <div className="relative">
             ...
             <Input 
              type="number"
              placeholder="0"
              value={hotelMadinah?.pricePerNight || ''}
              onChange={(e) => updateHotelMadinah({ 
                hotelId: 'custom', 
      ...
      <div className="space-y-4 pt-4 border-t border-white/5">
        <h3 className="text-sm font-medium text-white/80">Rekomendasi Hotel Madinah</h3>
        <div className="grid gap-4">
          {madinahHotels.map((hotel) => {
            const isSelected = hotelMadinah?.hotelId === hotel.id;
            
            return (
              <div 
                key={hotel.id}
                onClick={() => updateHotelMadinah({ 
                  hotelId: hotel.id, 
                  name: hotel.name, 
                  stars: hotel.stars, 
      ...
      <div className="pt-6 mt-4 flex justify-between items-center">
        ...
        <Button 
          onClick={handleNext} 
          disabled={!hotelMadinah?.hotelId && !hotelMadinah?.pricePerNight} 
          size="lg" 
        ...
        >
          Lanjut ke Transportasi <ChevronRight className="w-4 h-4 ml-1 inline-block" />
        </Button>
      </div>
    </div>
  );
